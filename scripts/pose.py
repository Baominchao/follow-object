...
net = poseNet("resnet18-body", threshold = 0.2)
...
if __name__ == "__main__":
    ...
    detection_pub = rospy.Publisher('detected_objects', ObjectsStamped, queue_size=1)
    ...
    while True:
        ...
        objects_stamped_msg.objects = coordinate_transfor.parse_to_pose_estimation(poses, depth_data)
        detection_pub.publish(objects_stamped_msg)

        output.Render(img)
        output.SetStatus("{:s} | Network {:.0f} FPS".format("Pose Estimation", net.GetNetworkFPS()))
