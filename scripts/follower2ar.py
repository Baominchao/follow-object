...
class Follower():
    def __init__(self):
        ...
        # Subscribe to the zed2i ar tag topic 
        self.depth_subscriber = rospy.Subscriber('ar_tag', Marker, self.set_cmd_vel, queue_size=1)
        rospy.loginfo("Subscribing to  ar tag topic...")

        # Wait for the pointcloud topic to become available
        rospy.wait_for_message('ar_tag', Marker)
    ...
    def set_cmd_vel(self, msg):
        ...
        tracking_makrer_id =msg.id
        # If we have points, compute the centroid coordinates
        print ("We have found ar_track_alvar Marker No:", tracking_makrer_id)
        print ("curent marker: " +str(tracking_makrer_id))
        if tracking_makrer_id == 24:
            ...
            if (abs(z - self.goal_z) > self.z_threshold):
                if ((abs(z - self.goal_z) - self.z_threshold) > 0.1):
                    linear_speed = (z - self.goal_z) * self.z_scale*1.2
                else:
                    linear_speed = (z - self.goal_z) * self.z_scale*0.5
            ...
            if (abs(x) > self.x_threshold):  
                if ((abs(x) - self.x_threshold) > 0.05):
                    angular_speed = -x * self.x_scale*1.5
                else:
                    angular_speed = -x * self.x_scale*0.4
        ...
        if abs(self.move_cmd.linear.x) < 1e-5 and abs(self.move_cmd.angular.z)  < 1e-5:
            self.move_cmd = Twist()
